...
import { XmlService, XmlContent } from "../../services/xml.service";
...
export class GithubRepoManagerComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (token) {
      this.githubInfoForm.patchValue({ githubToken: token });
  ...
  initForms(): void {
    ...
    this.apiInfoForm = this.fb.group({
      xmlApiUrl: ["", [Validators.required, Validators.pattern("https?://.+")]],
    });
  }
      ...
      this.githubService
        ...
        .pipe(
          catchError((error) => {
            env.status = "error";
            this.showMessage(
              `Failed to create ${env.name} repository: ${error.message}`
            );
            return of(null);
          }),
          finalize(() => {
  ...
  fetchAndCommitXmls(): void {
    ...
    if (!username || !token || !xmlApiUrl) {
      this.showMessage("Missing required GitHub information");
      return;
    }

    this.isLoading = true;
    let completedCount = 0;
    const envCount = this.environments.filter(
      (env) => env.status === "created"
    ).length;
    ...
    for (const env of this.environments) {
      ...
      this.xmlService
        .fetchXmlsFromApi(`${xmlApiUrl}/${env.name.toLowerCase()}`)
        .pipe(
          switchMap((xmls: XmlContent[]) => {
            env.xmlCount = xmls.length;

            // Create an array of observables for committing each XML
            const commitObservables = xmls.map(
              (xml: XmlContent, index: number) => {
                return this.githubService
                  .addFileToRepo(
                    username,
                    env.repoName as string,
                    `xml-${index + 1}.xml`,
                    xml.content || xml, // Handle both string and object content
                  ...
                  .pipe(
                    catchError((error) => {
                      console.error(
                        `Failed to commit XML ${index + 1} to ${
                          env.name
                        } repo:`,
                        error
                      );
          ...
          catchError((error) => {
            env.status = "error";
            this.showMessage(
              `Failed to fetch XMLs for ${env.name}: ${error.message}`
            );
            return of([]);
          }),
          finalize(() => {
            ...
            // Update loading state after all environments are processed
            completedCount++;
            if (completedCount === envCount) {
              this.isLoading = false;
              this.currentStep = 3;
            }
          })
        ...
        .subscribe((results) => {
          ...
          if (successfulCommits > 0) {
            this.showMessage(
              `Committed ${successfulCommits} XML files to ${env.name} repository`
            );
          }
        });
    }
      ...
      this.githubService
        ...
        .pipe(
          catchError((error) => {
            env.status = "error";
            this.showMessage(
              `Failed to create version for ${env.name}: ${error.message}`
            );
            return of(null);
          }),
          finalize(() => {
        ...
        .subscribe((tag) => {
          if (tag) {
            this.showMessage(
              `Created version v1.0.0 for ${env.name} repository`
            );
          }
        });
    }
